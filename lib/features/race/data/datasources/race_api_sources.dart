...
class RaceApiSources {
  ...
  Future<Race> createRace(Race race) async {
    try {
      ...
      raceJson['RAC_MAX_TEAM_MEMBERS'] = raceJson.remove('RAC_MAX_TEAM_MEMBERS');
      ...
      print('🏁 CreateRace - Token présent: ${_authToken != null}');
      print('🏁 CreateRace - Body: $body');
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        ...
        throw Exception('Non authentifié - Vous devez être connecté via l\'API');
      } else if (response.statusCode == 403) {
        throw Exception('Accès refusé - Seul le gestionnaire du raid peut créer des courses');
      } else if (response.statusCode == 422) {
        final errorData = json.decode(response.body);
        throw Exception('Erreur de validation : ${errorData['errors'] ?? response.body}');
      } else {
